...
import FormStepPanel, { Step } from './form-step-panel';
...
describe('FormStepPanel Component', () => {
  test('renders steps with correct labels', () => {
    render(<FormStepPanel steps={steps} activeStepId="step1" overallStatusIcon="bi-clock" overallTitle="The loan" />);
  ...
  //   render(<FormStepPanel steps={steps} activeStepId="step2" overallStatusIcon="bi-clock" overallTitle="The loan" />);
  ...
  test('calls onStepClick when a step is clicked', () => {
    ...
    render(<FormStepPanel steps={steps} activeStepId="step1" onStepClick={handleStepClick} overallStatusIcon="bi-clock" overallTitle="The loan" />);
  ...
  test('handles undefined activeStepId', () => {
    const handleStepClick = jest.fn();
    render(<FormStepPanel steps={steps} activeStepId={undefined} onStepClick={handleStepClick} overallStatusIcon="bi-clock" overallTitle="The loan" />);
  ...
  test('toggles the panel when the toggle button is clicked (vertical orientation)', () => {
    const { rerender } = render(<FormStepPanel steps={steps} activeStepId="step1" orientation="vertical" collapsed={false} overallStatusIcon="bi-clock" overallTitle="The loan" />);
    const toggleButton = screen.getByRole('button', { name: /toggle panel/i });
    
    fireEvent.click(toggleButton);
    rerender(<FormStepPanel steps={steps} activeStepId="step1" orientation="vertical" collapsed={true} overallStatusIcon="bi-clock" overallTitle="The loan" />);
    const stepPanel = toggleButton.closest('.step-indicator');
    expect(stepPanel).toHaveClass('collapsed');

    fireEvent.click(toggleButton);
    rerender(<FormStepPanel steps={steps} activeStepId="step1" orientation="vertical" collapsed={false} overallStatusIcon="bi-clock" overallTitle="The loan" />);
  ...
  test('does not render collapse button in horizontal orientation', () => {
    render(<FormStepPanel steps={steps} activeStepId="step1" orientation="horizontal" overallStatusIcon="bi-clock" overallTitle="The loan" />);
