...
describe('StepPanel Component', () => {
  test('renders steps with correct labels', () => {
    render(<StepPanel steps={steps} activeStepId="step1" />);
  ...
  test('applies active class to the active step', () => {
    render(<StepPanel steps={steps} activeStepId="step2" />);
  ...
  test('calls onStepClick when a step is clicked', () => {
    ...
    render(<StepPanel steps={steps} activeStepId="step1" onStepClick={handleStepClick} />);
  ...
  test('handles undefined activeStepId', () => {
    const handleStepClick = jest.fn();
    render(<StepPanel steps={steps} activeStepId={undefined} onStepClick={handleStepClick} />);
  ...
  test('toggles the panel when the toggle button is clicked (vertical orientation)', () => {
    const { rerender } = render(<StepPanel steps={steps} activeStepId="step1" orientation="vertical" collapsed={false} />);
  ...
  test('does not render collapse button in horizontal orientation', () => {
    render(<StepPanel steps={steps} activeStepId="step1" orientation="horizontal" />);
